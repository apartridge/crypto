module crypto.blockcipher.taes;
...
class TableAES
{
    ...
    uint[] state;
    uint[Nb*(Nr+1)] w; // Key
    uint[Nb*(Nr+1)] dw; // Inverse key with precomputed InvMixColumns
    ...
    public void encrypt(ubyte[] message)
    {
        ...
        // Add initial round key (should be automatically unrolled)
        for (int i = 0; i < Nb; ++i)
            state[i] ^= w[i];

        // Temporaries between each round (should be register allocated)
        uint[4] tmp;
        uint round = 0;
        ...
        while (round++ < Nr - 1)
        {
            ...
            // Can use rot instruction to get 1KB of table
            // TODO: Manually unroll ?
            tmp[] = state[0 .. Nb];
            for (uint col = 0; col < Nb; ++col)
            {
                uint a = tmp[col] & 0xff;
                uint b = (tmp[(col+1) % Nb] >> 8) & 0xff;
                uint c = (tmp[(col+2) % Nb] >> 16) & 0xff;
                uint d = (tmp[(col+3) % Nb] >> 24) & 0xff;
                state[col] = t1[a] ^ t2[b] ^ t3[c] ^ t4[d] ^ w[round*Nb + col];
                //state[col] = t1[a] ^ rotate!1(t1[b]) ^ rotate!2(t1[c]) ^ rotate!3(t1[d]) ^ w[round*Nb + col];
            }
            //printHexInt(round+1, "start", state);
        }

        // SubBytes and ShiftRows
        // Can eliminate sbox by masking T-table
        tmp[] = state[0 .. Nb];
        for (uint col = 0; col < Nb; ++col)
        {
    ...
    public void decrypt(ubyte[] cipher)
    {
        ...
        for (int round = Nr - 1; round > 0; --round)
        {
            ...
            {
                uint a = tmp[col] & 0xff;
                uint b = (tmp[(col+3) % Nb] >> 8) & 0xff;
                uint c = (tmp[(col+2) % Nb] >> 16) & 0xff;
                uint d = (tmp[(col+1) % Nb] >> 24) & 0xff;
                state[col] = it[a] ^ rotate!1(it[b]) ^ rotate!2(it[c]) ^ rotate!3(it[d]) ^ dw[round*Nb + col];
            }
        ...
        for (uint col = 0; col < Nb; ++col)
        {
            uint a = inv_sbox[tmp[col] & 0xff];
            uint b = inv_sbox[(tmp[(col+3) % Nb] >> 8) & 0xff] << 8;
            uint c = inv_sbox[(tmp[(col+2) % Nb] >> 16) & 0xff] << 16;
            uint d = inv_sbox[(tmp[(col+1) % Nb] >> 24) & 0xff] << 24;
            state[col] = a ^ b ^ c ^ d ^ dw[col];
        }
    ...
    unittest
    {
        ...
        auto aes = new TableAES(key);

        aes.encrypt(buffer);
        assert(buffer == cipher, byteToHexString(buffer));

        aes.decrypt(buffer);
        assert(buffer == message, byteToHexString(buffer));
    }
    ...
    unittest
    {
        assert(rotate!3(0x0b0d090e) == 0x0e0b0d09);
    }
...
void tableAESspeedBenchmark()
{
    auto blockCipher = new TableAES(cast(ubyte[16]) x"63cab7040953d051cd60e0e7ba70e18c");
    auto message = new ubyte[16];
    auto outputBuffer = new ubyte[16];

    std.stdio.writeln("Running T-AES benchmark");

    int megaBytes = 10;
    int iterations = megaBytes*1024*1024 / 16;

    long tStart = std.datetime.Clock.currStdTime();
    ...
    long tEnd = std.datetime.Clock.currStdTime();
    long encryptTime = tEnd - tStart;

    std.stdio.write("Encryption time: "); std.stdio.write(encryptTime / 10000000.0); std.stdio.writeln(" seconds");
    blockCipher.reportTiming(encryptTime);
    std.stdio.write("Throughput: "); std.stdio.write(megaBytes / (encryptTime / 10000000.0)); std.stdio.writeln(" MB/s");
}

// Validate output against openssl to check byte ordering is not messed up
void testAesFile()
{
    auto blockCipher = new TableAES(cast(ubyte[16]) x"000102030405060708090a0b0c0d0e0f");
